...
public class MapEditorController {
    ...
    private void printTileSize(String where) {
        for (Node node : editorGrid.getChildren()) {
            if (node instanceof EditorTile editorTile) {
                LOGGER.info("TileSize in {}: {}x{}", where,
                            editorTile.getWidth(), editorTile.getHeight());
            }
        }
    }
    ...
    private EditorTile createEditorTile(int width, int height, String tileType) {
        ...
        editorTile.heightProperty()
                  .addListener((observable, oldValue, newValue) -> {
                      LOGGER.info("Height changed from {} to {} - {}", oldValue,
                                  newValue, observable);
                  });

        double tileMaxHeight =
                Context.getPrimaryStage().getHeight() / height / 100 * 80;
        double tileMaxWidth =
                Context.getPrimaryStage().getWidth() / width / 100 * 80;
